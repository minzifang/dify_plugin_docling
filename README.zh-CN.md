...
如果你的网关把解析接口暴露在其它路径，可以在 Provider 凭据里设置 `解析接口路径`。例如完整解析地址是 `http://192.168.4.211:5009/` 时，填写：
...
| `解析接口路径` | 否 | 拼接到 `Docling API 地址` 后面的文件解析路径，默认 `/v1/convert/file`。如果你的代理把解析接口暴露在根路径，请填 `/`。 |
